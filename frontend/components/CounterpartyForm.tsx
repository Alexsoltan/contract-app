  ...
  return (
    <div className="space-y-5">
      ...
      <Input
        label="ИНН"
        value={form.inn || ""}
        onChange={(v) => updateForm("inn", v)}
      />

      <Input
        label="КПП"
        value={form.kpp || ""}
        onChange={(v) => updateForm("kpp", v)}
      />

      <Input
        label="ОГРН / ОГРНИП"
        value={form.ogrn || ""}
        onChange={(v) => updateForm("ogrn", v)}
      />

      <Input
        label="Юридический адрес"
        value={form.legal_address || ""}
        onChange={(v) => updateForm("legal_address", v)}
      />

      <Input
        label="Банк"
        value={form.bank_name || ""}
        onChange={(v) => updateForm("bank_name", v)}
      />

      <Input
        label="БИК"
        value={form.bik || ""}
        onChange={(v) => updateForm("bik", v)}
      />

      <Input
        label="Расчётный счёт"
        value={form.payment_account || ""}
        onChange={(v) => updateForm("payment_account", v)}
      />

      <Input
        label="Корреспондентский счёт"
        value={form.correspondent_account || ""}
        onChange={(v) => updateForm("correspondent_account", v)}
      />

      <Input
        label="ФИО подписанта"
        value={form.signer_full_name || ""}
        onChange={(v) => updateForm("signer_full_name", v)}
      />

      <Input
        label="Должность подписанта"
        value={form.signer_position || ""}
        onChange={(v) => updateForm("signer_position", v)}
      />

      <Input
        label="Основание полномочий"
        value={form.signer_basis || ""}
        onChange={(v) => updateForm("signer_basis", v)}
      />
    </div>
  );
}